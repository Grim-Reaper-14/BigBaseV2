...
namespace rage
{
	...
	// pgBase vtable/page-map region; keeping them opaque lets this type also be
	// used as the temporary native-table initializer required by Enhanced.
	struct scrProgram final
	{
		...
		[[nodiscard]] std::uint32_t code_page_size(std::uint32_t page) const noexcept
		{
			...
			return tail ? tail : scr_program_page_size;
		}
		...
		[[nodiscard]] std::uint8_t* code_page(std::uint32_t page) const noexcept
		{
			if (!m_code_blocks || page >= code_page_count())
				return nullptr;
		...
		[[nodiscard]] std::uint8_t* code_address(std::uint32_t index) const noexcept
		{
			...
			auto* page = m_code_blocks[index >> 14];
