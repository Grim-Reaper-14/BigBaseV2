...
namespace big::menu_pages
{
	namespace
	{
		...
		const char* status_text(vehicle_spawn_status status) noexcept
		{
			switch (status)
			{
			case vehicle_spawn_status::idle:
				return "Ready";
			case vehicle_spawn_status::loading:
				return "Loading model...";
			case vehicle_spawn_status::spawned:
				return "Vehicle spawned";
			case vehicle_spawn_status::invalid_model:
				return "Invalid vehicle model";
			case vehicle_spawn_status::load_failed:
	...
	void draw_vehicle()
	{
		...
		ImGui::InputText("Model", g_vehicle_settings.spawn_model.data(), g_vehicle_settings.spawn_model.size());
		...
		if (ImGui::Button("Adder"))
			set_preset("adder");
		ImGui::SameLine();
		if (ImGui::Button("Zentorno"))
			set_preset("zentorno");
		ImGui::SameLine();
		if (ImGui::Button("Kuruma"))
			set_preset("kuruma");
		ImGui::SameLine();
		if (ImGui::Button("Oppressor"))
			set_preset("oppressor");

		if (ImGui::Button("Spawn Vehicle", ImVec2(190.0f, 0.0f)))
		{
			queue_vehicle_spawn(
		...
		{
			// Action wiring is handled by the vehicle feature layer.
		}
		ImGui::SameLine();
